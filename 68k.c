...
void M68K_AluBusUpdateStrong(m68k_t *chip)
{
    ...
    if (chip->c6)
    {
        chip->b2[0].val &= ~chip->b2[1].val;
        chip->b2[1].val &= ~chip->b2[0].val;
        chip->b2[2].val &= ~(chip->b2[3].val & 0xff);
        chip->b2[3].val &= ~(chip->b2[2].val & 0xff);
    }

    if (chip->w857)
    {
        chip->b2[2].val &= ~(chip->b2[3].val & 0xff00);
        chip->b2[3].val &= ~(chip->b2[2].val & 0xff00);
    }
...
void M68K_Clock(m68k_t* chip, int clk1, int clk2)
{
    ...
    chip->w548 = !(chip->tm_w1 || !chip->w605);
    ...
    if (chip->w600)
    {
        chip->w609 = !chip->tm_w1;
        chip->w610 = !chip->tm_w2;
        chip->w611 = !chip->tm_w3;
    }
    else if (chip->w604)
    {
    ...
    chip->w651 = !(chip->w652 ? chip->tm_w1 : chip->w572);
    chip->w653 = !(chip->w652 ? chip->tm_w1 : chip->w581);
    chip->w654 = !(chip->w652 ? chip->tm_w1 : chip->w580);
    chip->w655 = !(chip->w652 ? chip->tm_w1 : chip->w579);
    ...
    chip->w734 = !chip->tm_w1;
    chip->w735 = !chip->tm_w1;
    ...
    if (!chip->w723)
        chip->w739 = 0;
    if (!chip->w720)
        chip->w739 = chip->tm_w1;

    if (chip->c2)
        chip->w740 = !chip->tm_w1;

    if (chip->c2)
        chip->w741 = !chip->tm_w1;
    ...
    chip->w743 = !chip->tm_w1;

    if (!chip->w714)
        chip->w744 = chip->w743;
    if (!chip->w716)
        chip->w744 = chip->tm_w1;
    if (!chip->w715)
        chip->w744 = chip->tm_w2;
    ...
    if (!chip->w771 && !chip->w789)
        chip->w784 = chip->tm_w1;
    if (!chip->w772 && !chip->w789)
        chip->w784 = chip->tm_w2;
    if (chip->w786)
        chip->w784 = chip->w781;
    ...
    if (chip->w772)
        chip->w796 = chip->tm_w1;
    if (chip->w771)
        chip->w796 = chip->tm_w2;
    ...
    if (chip->w772)
        chip->w798 = chip->tm_w1;
    if (chip->w771)
        chip->w798 = chip->tm_w2;
    ...
    chip->w802 = !chip->tm_w1;
    chip->w803 = !(chip->tm_w1 && chip->tm_w2);
