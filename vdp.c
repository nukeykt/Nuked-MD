...
void VDP_ClockSprites(vdp_t *chip, int clk1, int clk2)
{
    ...
    chip->w698 = chip->l367[1] && chip->tm_w2 && chip->tm_w3;
    ...
    chip->w705 = (chip->reg_test_18 & 96) == 32;
    chip->w706 = (chip->reg_test_18 & 96) == 64;
    chip->w707 = (chip->reg_test_18 & 96) == 96;

    chip->w708 = chip->w96 && chip->tm_w2 && chip->tm_w3;
    chip->w709 = chip->w708 || chip->w713;

    chip->w708 = chip->w96 && chip->tm_w2 && chip->tm_w3;

    chip->w711 = chip->w96 && chip->tm_w2 && chip->tm_w3;
    ...
    chip->w746 = chip->l393[0] && clk2;

    chip->w747 = !(chip->l396[1] && chip->hclk1);
    chip->w748 = !(chip->l397[1] && chip->hclk1);
    ...
    chip->w787 = (chip->l433 + chip->tm_w1 + chip->w785) & 15;
    ...
    chip->w788 = chip->tm_w1 ? chip->l436 : chip->l435;
    ...
    chip->w790 = chip->w789 && chip->tm_w1;
    ...
    chip->w804 = chip->l456[1] && chip->tm_w1;

    chip->w805 = chip->l456[1] && chip->tm_w2 && chip->hclk1;
    ...
    chip->w817 = chip->l464[1] || chip->w816 || chip->tm_w1 || (chip->reg_test0 & 8192) != 0;

    chip->w818 = !(chip->tm_w1 && (chip->reg_test0 & 8192) == 0);
    ...
    if (chip->vram_data & 2)
        chip->vram_serial_swap1 |= 8;
    if (chip->vram_data & 4)
        chip->vram_serial_swap1 |= 64;
    if (chip->vram_data & 8)
        chip->vram_serial_swap1 |= 4;
    if (chip->vram_data & 16)
        chip->vram_serial_swap1 |= 32;
    if (chip->vram_data & 32)
        chip->vram_serial_swap1 |= 2;
    if (chip->vram_data & 64)
        chip->vram_serial_swap1 |= 16;
    if (chip->vram_data & 128)
        chip->vram_serial_swap1 |= 1;

    if (chip->w834)
    {
        chip->l478 = chip->vram_serial_swap1;
    }
    if (chip->w833)
    {
        chip->l479 = chip->vram_serial_swap1;
    }
    if (chip->w829)
    {
        chip->l480 = chip->vram_serial_swap1;
    }
    if (chip->w830)
    {
        chip->l481 = chip->vram_serial_swap1;
    }
    ...
    if (clk1)
    {
        ...
        chip->l504 = ((chip->l503[1] << 1) ^ 2) | chip->l506[1];
    ...
    {
        chip->w844 = (chip->w843 >> 4) & 15;
        chip->w844 |= (chip->w843 << 4) & 0xf0;
    }
    ...
    chip->w856 = !(chip->w852 && chip->w816);
    chip->w857 = !(chip->w853 && chip->w816);
    chip->w858 = !(chip->w854 && chip->w816);
    chip->w859 = !(chip->w855 && chip->w816);
...
void VDP_ClockVRAMCtrl(vdp_t *chip, int clk1, int clk2)
{
    ...
    if (chip->w998)
    {
        chip->l598 = chip->tm_w1;
    }

    if (chip->w998)
    {
        chip->l599 = chip->tm_w1;
    }
