...
void Z80_Clock(z80_t *chip, int clk)
{
    ...
    chip->w117 = chip->tm_w1 || chip->tm_w2 || (chip->tm_w3 && chip->tm_w4) || (chip->tm_w10 && chip->w138 && chip->tm_w12)
        || (chip->tm_w7 && (chip->tm_w5 || (chip->tm_w6 && (!(chip->tm_w4 || !chip->w151) || !chip->w151))));
    ...
    chip->w142 = !chip->tm_w1 && !chip->tm_w2 && !chip->tm_w3;
    ...
    chip->w203 = !(chip->w110 || (chip->w141 && chip->w131));

    chip->w204 = !((chip->w109 && chip->pla[93])
        || (chip->pla[88] && chip->w121 && chip->w141));
    ...
    chip->w219 = !((chip->w141 && (chip->w121 || (chip->w127 && !chip->w186)))
        || (chip->w109 && chip->w131)
